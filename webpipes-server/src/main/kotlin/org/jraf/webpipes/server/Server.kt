...
private const val PATH_EXECUTOR_ID = "executorId"

class Server(
  private val executors: Map<String, suspend (RequestParams) -> String>,
) {
  fun start() {
  ...
  private fun Application.mainModule() {
    ...
    install(StatusPages) {
      status(HttpStatusCode.NotFound) { call, status ->
        call.respondText(
          text = "Usage: ${call.request.local.scheme}://${call.request.local.serverHost}:${call.request.local.serverPort}/<executorId>\n\nSee https://github.com/BoD/WebPipes for more info.",
          status = status,
        )
      }
    }
    ...
    routing {
      get("{$PATH_EXECUTOR_ID}") {
        val executorId = call.parameters[PATH_EXECUTOR_ID]!!
        val executor = executors[executorId] ?: run {
